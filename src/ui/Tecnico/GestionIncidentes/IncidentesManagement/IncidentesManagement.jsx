...
const IncidentesManagement = () => {
  ...
  const [formData, setFormData] = useState({
    estado: 2, // Default to Resuelto for technicians
    comentarios_tecnico: '',
  });
  ...
  const handleEditSubmit = async (e) => {
    ...
    const validateForm = () => {
      const newErrors = {};
      if (userRole === 3 && formData.estado !== 2) {
        newErrors.estado = 'El estado debe ser Resuelto';
      }
      setErrors(newErrors);
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Comentarios Técnico</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan="8" className="px-6 py-4 text-center text-gray-600">No hay incidentes asignados</td>
                </tr>
              ) : (
                incidentes.map((incidente) => (
                  <tr key={incidente.idIncidente} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">{incidente.comentarios_tecnico || '-'}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
              ...
              <form onSubmit={handleEditSubmit}>
                <div className="mb-4">
                  ...
                  <select
                    ...
                    disabled
                    className="block w-full rounded-md border-gray-300 shadow-sm bg-gray-100 cursor-not-allowed sm:text-sm"
                  >
                ...
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="comentarios_tecnico">
                    Comentarios Técnico
                  </label>
                  <textarea
                    id="comentarios_tecnico"
                    value={formData.comentarios_tecnico}
                    onChange={(e) => setFormData({ ...formData, comentarios_tecnico: e.target.value })}
                    className="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                    rows="4"
                    placeholder="Ingrese comentarios técnicos (opcional)"
                  />
                  {errors.comentarios_tecnico && <p className="text-red-500 text-xs mt-1">{errors.comentarios_tecnico}</p>}
                </div>
